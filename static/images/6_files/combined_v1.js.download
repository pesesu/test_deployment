...
function togglePopup(targetId, event){

    event.preventDefault();
    event.stopPropagation();
    var styleObject = getStyleObject(targetId);

    if(styleObject != false){
        if(styleObject.visibility == 'hidden'){
            styleObject.visibility = 'visible';
            styleObject.opacity = 1;

            if(targetId == 'lang_option_list' ){
                var userPopup = document.getElementById('userMenu');
                if( userPopup != null && typeof userPopup.style !== 'undefined' && userPopup.style.display != 'none'){
                    show_user_menu();
                }
            }
        }else{
